...
def MainToolbar(root, parentpanel = None, use_wrapsizer = False):
    ...
    root.rescanbtn = make_autosize_button(parentpanel, _(u"端口"), root.rescanports, _("Communication Settings\nClick to rescan ports"))
    ...
    if not hasattr(root, "connectbtn"):
        root.connectbtn = make_autosize_button(parentpanel, _(u"连接"), root.connect, _("Connect to the printer"))
    ...
    # if not hasattr(root, "resetbtn"):
        # root.resetbtn = make_autosize_button(parentpanel, _(u"Reset"), root.reset, _("Reset the printer"))
    ...
    root.loadbtn = make_autosize_button(parentpanel, _(u"载入打印文件"), root.loadfile, _("Load a 3D model file"), self)
    ...
    if not hasattr(root, "printbtn"):
        root.printbtn = make_autosize_button(parentpanel, _(u"打印"), root.printfile, _("Start Printing Loaded File"))
    ...
    if not hasattr(root, "pausebtn"):
        root.pausebtn = make_autosize_button(parentpanel, _(u"暂停"), root.pause, _("Pause Current Print"))
        root.statefulControls.append(root.pausebtn)
    else:
        root.pausebtn.Reparent(parentpanel)
    self.Add(root.pausebtn)
    root.offbtn = make_autosize_button(parentpanel, _(u"停止打印"), root.off, _("Turn printer off"), self)
